...
Claude Code, and Codex in the same thread, reading the same history — and when
...
say plainly that they have no key rather than pretending to be switched on.
...
be permanently ambiguous with the agent of that name, with no honest way to
pick.

...
| **Continue an existing project** | Behind `CONTINUATION_WEDGE_ENABLED`, `/continue` brings a repository and at least one prior conversation together before starting work. It turns imported chats, pasted notes, documents, and URL references into a sourced brief; separates what is understood from the few consequential claims that need confirmation; and accepts that brief into one ordinary project thread idempotently. Sources retain freshness and limitation labels instead of being presented as equally current or verified. The same wedge adds explicit handoff receipts, read-only checkout conflict preflight, run evidence sheets, project search across decisions and confirmed claims, and a “since you left” cursor that advances only after the client acknowledges it. Web and native destinations share stable typed routes, while native continuation events and the first consequential context change are ready for APNs. |
...
| **Existing-project continuation wedge** | `CONTINUATION_WEDGE_ENABLED=true`; APNs keys for native alerts | The existing Inbox and import flows remain unchanged; continuation, checkout preflight, and evidence routes stay unavailable. |

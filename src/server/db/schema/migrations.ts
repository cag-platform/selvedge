...
import type { MigrationPlan, MigrationProjectMap, MigrationSource } from '../../../shared/types/migration.js';
...
export const migrationJourneys = pgTable('migration_journeys', {
  ...
  projectMap: jsonb('project_map').$type<MigrationProjectMap>().notNull(), migrationPlan: jsonb('migration_plan').$type<MigrationPlan>(), destinations: jsonb('destinations').notNull().default({}),

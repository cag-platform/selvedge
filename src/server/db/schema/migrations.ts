...
import type { MigrationOwnerTestFlow, MigrationPlan, MigrationProjectMap, MigrationSource, MigrationVerification } from '../../../shared/types/migration.js';
...
export const migrationJourneys = pgTable('migration_journeys', {
  ...
  projectMap: jsonb('project_map').$type<MigrationProjectMap>().notNull(), migrationPlan: jsonb('migration_plan').$type<MigrationPlan>(), migrationVerification: jsonb('migration_verification').$type<MigrationVerification>(), ownerTestFlow: jsonb('owner_test_flow').$type<MigrationOwnerTestFlow>(), destinations: jsonb('destinations').notNull().default({}),

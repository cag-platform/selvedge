...
export function ImportGithub({ source }: { source: Extract<MigrationSource, 'github' | 'codex' | 'claude-code' | 'cursor' | 'lovable'> }) {
  ...
  return <div className="mt-5 rounded-card border border-hairline bg-panel-soft p-4"><label className="text-meta text-ink-dim">Which repository holds the project?<select value={repo} onChange={(event) => setRepo(event.target.value)} disabled={busy} className="mt-1 block w-full rounded-inset border border-hairline bg-panel px-3 py-2.5 text-body text-ink">{repos.map((item) => <option key={item.full_name} value={item.full_name}>{item.full_name}</option>)}</select></label><button type="button" onClick={() => void start()} disabled={!repo || busy} className="mt-3 rounded-full bg-action px-5 py-2.5 text-body font-medium text-white disabled:opacity-40">{phase === 'inspecting' ? 'Inspecting repository…' : phase === 'preparing' ? 'Preparing preview…' : phase === 'opening' ? 'Opening migration…' : 'Let Selvedge migrate it →'}</button>{busy && <p className="mt-3 text-meta text-ink-dim">Selvedge is mapping the repository, preparing a temporary copy, and opening the project conversation. The source repository is not modified.</p>}{error && <p role="alert" className="mt-3 text-meta text-thread">{error}</p>}</div>;
}
